...
- **NetworkManager instead of iwd** on the wired reference install (no Wi-Fi radio): NetworkManager is the active daemon and iwd was removed in 2026-08 (radio-less box, so nothing gained by keeping it dormant). iwd remains the baseline where a radio exists.
