...
| Exit confirm | swaynag (`Mod+Shift+e`) | `swaynag` | bundled in `sway` | ✓ | ? |
| Screenshots | grim + slurp (`Ctrl+Alt+s`) | `grim` `slurp` | `grim` `slurp` | ✓ | ? |
| Lock / idle | swaylock + swayidle | `swaylock` `swayidle` | `swaylock` `swayidle` | ✓ | ? |
| Volume | `scripts/volume-control.sh` (wpctl → pactl → silent) | `wpctl` | `wireplumber` (ships wpctl) | ✓ | ? |
...
| Network daemon | iwd on Arch, NetworkManager on Debian | `iwd` | `network-manager` (ships `nmcli` **and** `nmtui`) | ✓ (NM) | ? (iwd) |
| Network TUI | `scripts/network-tui.sh`: impala → nmtui → iwctl → ip | `iwd`, impala via pipx | `networkmanager` (nmtui) | ✓ (nmtui) | ? |
| Firewall | ufw, SSH rate-limited before enable | `ufw` | `ufw` | ✗ | ? |
| Portals | file picker / screenshot / screencast (`wlr;gtk` preference) | `xdg-desktop-portal` `-wlr` `-gtk` | same three | ✓ (wlr+gtk+gnome backends) | ? |
| Clipboard | wl-clipboard (standard `Ctrl+C`/`V` in kitty) | `wl-clipboard` | `wl-clipboard` | ✓ | ? |
| SSH agent | Debian: gnome-keyring `ssh-agent.socket` (self-exports `SSH_AUTH_SOCK`); Arch: user unit from `bootstrap/user/` | `openssh` | `openssh-client` + gnome-keyring | ✓ | ? |
| Secrets | age-encrypted, identity = SSH ed25519 key | `age` `rage` | `age` (rage not packaged — encrypt on an Arch box) | ✗ (age not installed) | ? |
| Tailscale | joins via `secrets/tailscale_authkey` | `tailscale` | not in archive — vendor repo (pkgs.tailscale.com) | ✓ (vendor) | ? |
| System snapshots | snapper, btrfs-gated (default installs are ext4 → N/A) | `snapper` | `snapper` | ✗ (ext4 — N/A) | ? |
