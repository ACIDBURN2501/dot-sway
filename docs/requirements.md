...
| rage | not packaged; decrypting secrets only needs `age`, so encrypt on an Arch box |
...
- **Power** (`Super+Ctrl+p`, or the power pill ⚡'s left-click dropdown) → `scripts/quick-menu.sh power` → `extra/wofi/wofi-power.sh`; the dropdown is `waybar/menus/power.xml`, a parallel definition of `wofi-power.sh`, so keep the commands in sync
